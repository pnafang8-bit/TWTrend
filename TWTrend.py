...
def load_price_data():
query = “SELECT stock_id, trade_date, close FROM daily_price WHERE trade_date > CURRENT_DATE - INTERVAL ‘15 months’ ORDER BY stock_id, trade_date”
try:
...
@st.cache_data(ttl=3600)
def load_index_data():
query = “SELECT trade_date, close FROM tw_index WHERE trade_date > CURRENT_DATE - INTERVAL ‘15 months’ ORDER BY trade_date”
try:
...
results.append({“Stock”: stock_id, “Price”: curr_p, “Weighted_Ret”: weighted_ret, “High_1Y”: group[“close”].max()})
...
latest_rev = rev.sort_values(“year_month”).groupby(“stock_id”).tail(1)[[“stock_id”, “YoY”]]
...
radar_df = full_df[(full_df[“RS Score”] >= 90) & (full_df[“Explosive Setup”] == True) & (full_df[“Rev_YoY”] >= 0.3)].copy()
st.subheader(“10 倍股爆發雷達 (RS > 90 + 財報 + 趨勢)”)
st.dataframe(radar_df.style.format({“Price”: “{:.2f}”, “Rev_YoY”: “{:.2%}”}), use_container_width=True)
