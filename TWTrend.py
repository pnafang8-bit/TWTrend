...
# 1. 強制 IPv4 補丁 (解決 Streamlit Cloud 與 Supabase 的 DNS 解析問題)
...
# 3. 資料庫連線設定 (使用 URL 物件避免密碼中的 @ 符號導致解析錯誤)
# 確保 Host 是 pooler 位址，Port 是 6543
DB_URL = URL.create(
...
def load_price_data():
    ...
    try:
        ...
        if df.empty:
            st.warning("⚠️ 連線成功，但 daily_price 資料表目前沒有數據。")
            return pd.DataFrame()
        df["trade_date"] = pd.to_datetime(df["trade_date"])
        return df
    except Exception as e:
        st.error(f"❌ 股價資料讀取失敗: {str(e)}")
        return pd.DataFrame()
